...
import { shuffleArray, activateCardEffect, updateParty, updateEnemies, resolveEnemyAction } from '../utils/functions'
...
const CombatContainer = () => {
  ...
  React.useEffect(() => {
    ...
    foundEnemies = foundEnemies.enemies.map(e => {
      ...
      let attackIndex = 0
      if (rng > 6){
        attackIndex = Math.floor(Math.random() * actionStyles(e.actionStyle).length)
      }
      return {
        ...e,
        strength: 0,
  ...
  React.useEffect(async () => {
    if (turn === "endPlayerTurn"){
      ...
      for (let i = 0; i < enemiesDupe.length; i++){
        ...
        // grab enemy effect
        let actions = actionStyles(enemy.actionStyle, difficulty, enemy.modifier)
        ...
        // update enemy
        let newAttackIndex = enemy.attackIndex + 1 === actions.length ? 0 : enemy.attackIndex + 1
      ...
      if (!discardDupe.length){setDiscardPile(discardDupe)}
  ...
  React.useEffect(() => {
    ...
    let newHand = []
    for (let i = 0; i < 5; i++){newHand.push(playerDeck.shift())}
  ...
  const cardPlayed = (target) => {
    // false clicks
    if (!chosenCard.key){return null}
    if (chosenCard.target && target === null){return null}
    if (!chosenCard.target && target != null){return null}

    //card shouldn't be played if you don't have enough mana
    if (chosenCard.cost > mana){return null}
  ...
  return (
    <section className="combat-container">
      ...
      <EnemyContainer enemies={enemies} chosenCard={chosenCard} cardPlayed={cardPlayed}/>
      <PlayingField selectCard={selectCard} chosenCard={chosenCard} hand={hand} currentDeck={currentDeck} discardPile={discardPile} setTurn={setTurn}/>
    </section>
  )
}
